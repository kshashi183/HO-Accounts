...
dailyReportRouter.post('/dailyReportdata', async (req, res, next) => {
  ...
  try {
      dailyReportQuery(`SELECT p.*,p.RecdPVID as Unit_UId,p.Sync_HOId  as UpDated,DATE_FORMAT(p.Recd_PV_Date, "%d/%m/%Y") as date,
...
dailyReportRouter.post('/paymentVoucherDetails', async (req, res, next) => {
  ...
  try {
      dailyReportQuery(`SELECT p.* ,  m.UnitName,p.PVSrlID  as Unit_UId,p.Sync_HOId  as UpDated, DATE_FORMAT(p.Inv_date, "%d/%m/%Y") as date  
...
dailyReportRouter.post('/hoReceiptDetails', async (req, res, next) => {
    const date = req.body.date
  try {
      dailyReportQuery(`SELECT * FROM magodmis.ho_paymentrv_details
      WHERE Inv_date='${date}';`, (err,data) => {
          res.send(data)
      })
  } catch (error) {
      next(error)
...
dailyReportRouter.post('/hoReceipt', async (req, res, next) => {
    const date = req.body.date
  try {
      dailyReportQuery(`SELECT * FROM magodmis.payment_recd_voucher_register
...
dailyReportRouter.post('/taxSummary', async (req, res, next) => {
    const date = req.body.date
  try {
      dailyReportQuery(`SELECT d1.DC_InvType AS InvoiceType,IF( d1.TaxAmount>0,1,0) AS WithTax, d.Tax_Name AS TaxName,
...
dailyReportRouter.post('/productionSummary', async (req, res, next) => {
    const date = req.body.date
  try {
      dailyReportQuery(`SELECT d1.DC_InvType as InvoiceType,if( d1.TaxAmount >0,1,0) as WithTax, d1.ExNotNo as Ex_Not_no,
...
dailyReportRouter.post('/salesInvoice', async (req, res, next) => {
    const date = req.body.date
  try {
      dailyReportQuery(`SELECT d.*, m.UnitName,d.Dc_Inv_No as Unit_UId,d.Sync_HOId  as UpDated, DATE_FORMAT(d.Dc_inv_Date, "%d/%m/%Y") as date,
      ...
      WHERE m.Current AND  d.Dc_Inv_No= d1.Dc_Inv_No AND d.Inv_Date='${date}' GROUP BY  m.UnitName, d.Dc_Inv_No ORDER BY d.Inv_no;`, (err,data) => {
          res.send(data)
      })
  } catch (error) {
      next(error)
...
dailyReportRouter.post('/salesInvoiceDetails', async (req, res, next) => {
  ...
  try {
      ...
      WHERE d.Inv_Date='${date}' AND d.DC_Inv_No='${id}' AND m.Current;`, (err,data) => {
...
dailyReportRouter.post('/clearanceDetails', async (req, res, next) => {
  ...
  try {
      dailyReportQuery(`SELECT d1.*, d1.Id AS Unit_UId, m.UnitName, d1.Sync_HOId AS UpDated
  FROM magodmis.draft_dc_inv_register d
  ...
  AND d.DC_Inv_No = '${id}' AND m.Current;`, (err,data) => {
          res.send(data)
      })
  } catch (error) {
      next(error)
...
dailyReportRouter.post('/reportSummary', async (req, res, next) => {
  ...
  try {
      ...
      ELSE 'Excise'
        ...
        GROUP BY InvType ORDER BY InvType DESC;`, (err,data) => {
          res.send(data)
      })
  } catch (error) {
      next(error)
...
dailyReportRouter.post('/reportTaxDetails', async (req, res, next) => {
  ...
  FROM 
      magodmis.payment_recd_voucher_register
  WHERE Recd_PV_Date = '${date}';`, (err,data) => {
          res.send(data)
      })
  } catch (error) {
      next(error)
...
dailyReportRouter.post('/reportSummaryDetails', async (req, res, next) => {
  ...
  WHERE Inv_Date = '${date}'   
  GROUP BY
      DC_InvType ORDER BY Dc_InvType DESC;`, (err,data) => {
          res.send(data)
      })
  } catch (error) {
      next(error)
...
dailyReportRouter.post('/reportSummaryExpand', async (req, res, next) => {
  ...
  WHERE 
      Inv_Date = '${date}' AND  
      DC_InvType IN ('${getData}');`, (err,data) => {
          res.send(data)
      })
  } catch (error) {
      next(error)
...
dailyReportRouter.post('/reportSummaryTaxExpand', async (req, res, next) => {
  ...
  FROM
      magodmis.payment_recd_voucher_register
      WHERE Recd_PV_Date = '${date}'  AND  TxnType IN ('Cash');`, (err,data) => {
          res.send(data)
      })
  } catch (error) {
      next(error)
