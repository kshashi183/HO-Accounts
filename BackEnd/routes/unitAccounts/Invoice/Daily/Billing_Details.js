...
billingDetails.get('/getTab', (req,res)=>{
    const selectedDate = req.query.date;
    const sqlquery=`SELECT Inv_No , Net_Total, DC_InvType from magodmis.draft_dc_inv_register WHERE  Inv_Date = '${selectedDate}'`;
    misQuery(sqlquery, (err, data) => {
        if (err) {
            console.log("err in query", err);
        }
        else {
